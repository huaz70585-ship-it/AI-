...
| 品牌层 15% | 主色三档（亮/中/深）· 青色系归一（与 logo 同源） | `#2AA7B0` / `#0E7C86` / `#095E66` |
| 强调层 5% | 暖橙（促销/待办）/ 金额红 / 未读红点 / 灵光青（AI 专属点缀 ≤3%） | `#FF6A2B` / `#FF3B30` / `#FF4D4F` / `#14B8C4` |

铁律：冷色管信任，暖色管动作，两者不平分面积。页面底用 `#F6F7F9` 而非纯白，避免白卡片浮起显脏。
唯一色源是 `src/styles/tokens.css`，各视图禁止写死品牌色值。
